...
const verifyQueue = async (
  ...
  if (queues.length === 1) {
    ...
    console.log("DEBUG WEBHOOK - Checking integration conditions:");
    console.log("  - msg.key.fromMe:", msg.key.fromMe);
    console.log("  - ticket.isGroup:", ticket.isGroup);
    console.log("  - queues[0]?.integrationId:", queues[0]?.integrationId);
    console.log("  - queues:", JSON.stringify(queues.map(q => ({id: q.id, name: q.name, integrationId: q.integrationId}))));
    ...
    if (
      ...
      console.log("DEBUG WEBHOOK - Integration conditions MET! Getting integration...");
      const integrations = await ShowQueueIntegrationService(
        queues[0].integrationId,
        companyId
      );
      console.log("DEBUG WEBHOOK - Integration found:", JSON.stringify({id: integrations.id, type: integrations.type, urlN8N: integrations.urlN8N}));
      ...
      console.log("DEBUG WEBHOOK - handleMessageIntegration completed");
...
const flowbuilderIntegration = async (
  ...
  */
  ...
  if (
    ...
    String(whatsapp.flowIdNotPhrase) !== 'false'
  ) {
...
export const handleMessageIntegration = async (
  ...
  console.log("DEBUG WEBHOOK - handleMessageIntegration called");
  console.log("  - Integration type:", queueIntegration.type);
  console.log("  - Integration URL:", queueIntegration?.urlN8N);
  console.log("  - Message key:", msg.key);

  if (queueIntegration.type === "n8n" || queueIntegration.type === "webhook") {
    if (queueIntegration?.urlN8N) {
      console.log("DEBUG WEBHOOK - Sending to webhook URL:", queueIntegration.urlN8N);
      const options = {
        method: "POST",
      ...
      try {
        request(options, function (error, response) {
          if (error) {
            console.error("DEBUG WEBHOOK - Error sending:", error);
            throw new Error(error);
          } else {
            console.log("DEBUG WEBHOOK - Response status:", response?.statusCode);
            console.log("DEBUG WEBHOOK - Response body:", response?.body);
          }
        });
      } catch (error) {
        throw new Error(error);
...
const handleMessage = async (
  ...
  try {
    ...
    // Verificar cola y configurar integración si es necesario
    if (
      !ticket.isGroup &&
      !msg.key.fromMe &&
      !ticket.userId &&
      ...
      console.log("DEBUG - Verificando integración para ticket:", {
        ticketId: ticket.id,
        hasQueue: !!ticket.queue,
        queueId: ticket.queueId,
        queueName: ticket.queue?.name,
        queueIntegrationId: ticket.queue?.integrationId,
        contactName: contact.name,
        contactNumber: contact.number
      });
      
      // Si tiene cola asignada, verificar si la cola tiene integración
      if (ticket.queueId) {
        // Buscar la cola con su integración
        const queue = await Queue.findByPk(ticket.queueId, {
          attributes: ['id', 'name', 'integrationId']
        });
        
        console.log("DEBUG - Cola encontrada:", {
          queueId: queue?.id,
        ...
        if (queue && queue.integrationId) {
          console.log("DEBUG - Activando integración para ticket con cola:", ticket.id);
          await ticket.update({
            useIntegration: true,
            integrationId: queue.integrationId
          });
          ...
          console.log("DEBUG - Cola sin integrationId configurado");
        }
      } else if (!ticket.queue) {
        console.log("DEBUG - Ticket sin cola, ejecutando verifyQueue");
        // Si no tiene cola, ejecutar verifyQueue normal
        await verifyQueue(wbot, msg, ticket, contact);
      } else {
        console.log("DEBUG - Ticket con cola pero sin integrationId en la cola");
      }
