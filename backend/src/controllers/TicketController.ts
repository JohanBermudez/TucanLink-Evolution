...
export const index = async (req: Request, res: Response): Promise<Response> => {
  ...
  if (queueIdsStringified) {
    const parsedQueueIds = JSON.parse(queueIdsStringified);
    // Handle "null" string for tickets without queue
    queueIds = parsedQueueIds.filter((id: any) => id !== "null").map((id: any) => Number(id));
    if (parsedQueueIds.includes("null")) {
      queueIds.push(null as any);
    }
  }
